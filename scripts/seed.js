...
// Set env before importing PrismaClient
process.env.DATABASE_URL = 'file:' + dbPath;

// Dynamic require after setting env
...
async function main() {
    ...
    const mgrPerms = allPerms.filter(p => p.code.includes('.view') || p.code === 'leave.manage' || p.code === 'leave.request');
    ...
    const empPerms = allPerms.filter(p => ['leave.request', 'announcements.view', 'attendance.view'].includes(p.code));
    ...
    const emp1 = await prisma.employee.upsert({
        where: { id: 1 }, update: {},
        create: { empNo: 'EMP-001', firstName: 'Dennis', lastName: 'Sotto', email: 'sottodennis@gmail.com', phone: '+971 50 123 4567', department: 'Management', position: 'System Administrator', joinDate: '2024-01-15', status: 'Active' },
    });
    const emp2 = await prisma.employee.upsert({
        where: { id: 2 }, update: {},
        create: { empNo: 'EMP-002', firstName: 'Sarah', lastName: 'Ahmed', email: 'employee@mabdc.org', phone: '+971 55 987 6543', department: 'Human Resources', position: 'HR Coordinator', joinDate: '2024-03-01', status: 'Active' },
    });
    await prisma.employee.upsert({
        where: { id: 3 }, update: {},
        create: { empNo: 'EMP-003', firstName: 'Ahmed', lastName: 'Al Mansouri', email: 'ahmed.mansouri@company.ae', phone: '+971 52 111 2233', department: 'Engineering', position: 'Senior Developer', joinDate: '2024-02-10', status: 'Active' },
    });
    await prisma.employee.upsert({
        where: { id: 4 }, update: {},
        create: { empNo: 'EMP-004', firstName: 'Fatima', lastName: 'Al Hashimi', email: 'fatima.hashimi@company.ae', phone: '+971 56 444 5566', department: 'Finance', position: 'Finance Manager', joinDate: '2023-11-20', status: 'Active' },
    });
    await prisma.employee.upsert({
        where: { id: 5 }, update: {},
        create: { empNo: 'EMP-005', firstName: 'Mohammed', lastName: 'Khan', email: 'mohammed.khan@company.ae', phone: '+971 58 777 8899', department: 'Operations', position: 'Operations Lead', joinDate: '2024-04-05', status: 'Active' },
    });
    ...
    const adminUser = await prisma.user.upsert({
        where: { email: 'sottodennis@gmail.com' }, update: {},
        create: { email: 'sottodennis@gmail.com', passwordHash: hash, displayName: 'Dennis Sotto', employeeId: emp1.id },
    });
    const empUser = await prisma.user.upsert({
        where: { email: 'employee@mabdc.org' }, update: {},
        create: { email: 'employee@mabdc.org', passwordHash: hash, displayName: 'Sarah Ahmed', employeeId: emp2.id },
    });
    ...
    await prisma.userRole.upsert({
        ...
        update: {}, create: { userId: adminUser.id, roleId: superAdmin.id },
    });
    await prisma.userRole.upsert({
        where: { userId_roleId: { userId: empUser.id, roleId: employee.id } },
        update: {}, create: { userId: empUser.id, roleId: employee.id },
    });
    ...
    await prisma.announcement.upsert({
        where: { id: 1 }, update: {},
        create: { title: 'Welcome to MABDC HR Portal', body: 'We are excited to launch our new HR management system. All employees can now manage their leave requests, view attendance, and stay updated with company announcements.', priority: 'high' },
    });
    await prisma.announcement.upsert({
        where: { id: 2 }, update: {},
        create: { title: 'Q1 2026 Performance Reviews', body: 'Performance review cycle for Q1 2026 begins on March 1st. Please ensure all self-assessments are completed by March 15th.', priority: 'normal' },
    });
    ...
    for (let i = 0; i < 5; i++) {
        ...
        await prisma.attendance.upsert({
            where: { id: i + 1 }, update: {},
            create: { employeeId: emp2.id, date: dateStr, timeIn: '08:00', timeOut: i === 0 ? null : '17:00', totalHours: i === 0 ? null : 9, overtimeHours: i === 0 ? null : (i % 2 === 0 ? 1 : 0) },
        });
    }
